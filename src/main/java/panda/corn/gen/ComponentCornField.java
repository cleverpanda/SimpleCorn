...
public class ComponentCornField extends StructureVillagePieces.Village{
	...
	public boolean addComponentParts(World worldIn, Random rand, StructureBoundingBox box)
	{
		...
		{
			if(crop != null){
			this.setBlockState(worldIn,crop.getDefaultState().withProperty(BlockCorn.AGE, MathHelper.getRandomIntegerInRange(rand, 0, 3)), 1, 1, i, box);
			this.setBlockState(worldIn,crop.getDefaultState().withProperty(BlockCorn.AGE, MathHelper.getRandomIntegerInRange(rand, 0, 3)), 2, 1, i, box);
			this.setBlockState(worldIn,crop.getDefaultState().withProperty(BlockCorn.AGE, MathHelper.getRandomIntegerInRange(rand, 0, 3)), 4, 1, i, box);
			this.setBlockState(worldIn,crop.getDefaultState().withProperty(BlockCorn.AGE, MathHelper.getRandomIntegerInRange(rand, 0, 3)), 5, 1, i, box);
			}
			}
